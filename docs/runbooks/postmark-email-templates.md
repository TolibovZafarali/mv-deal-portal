# Postmark Email Templates (Shared Layout + CID Logo)
...
All templates use the same CID logo attachment:

```html
<img src="cid:mv-logo-white" alt="MV Deal Portal" />
```
...
## Send payload example (with CID logo)
...
Before production use of Postmark templates/layouts, add support for:

- `/email/withTemplate`
- `TemplateAlias`
- `TemplateModel`
- `Attachments` (for CID logo)
